...
polished_gpad['taxon'] = polished_gpad['taxon'].astype(str)
polished_gpad['query_taxid'] = polished_gpad['query_taxid'].astype(str)
...
def domain_gos(df, domain):
    ...
    df = df['go_name'].groupby(df['domain']).value_counts().rename('counts').reset_index()
    return df

def calc_the_diff(df1, df2):
    ''' Calculate the difference '''
    
    df = len(df1) - int(df2['counts'][:1].values + df2['counts'][1:2].values +
                       df2['counts'][2:3].values + df2['counts'][3:4])
    return df
...
euk_phylum = Euk['phylum'].groupby(Euk['domain']).value_counts().rename('counts').reset_index() # take top 5
bac_order = Bac['order'].groupby(Bac['domain']).value_counts().rename('counts').reset_index() # take top 5
vir_family = Vir['family'].groupby(Vir['domain']).value_counts().rename('counts').reset_index() # take top 5
arc_phylum = Arc['phylum'].groupby(Arc['domain']).value_counts().rename('counts').reset_index() # take top 2

## Calculate the difference of the total amount of the domain, and the subset top N rank 
euk_diff = len(Euk) - (int(euk_phylum['counts'][:1].values)+int(euk_phylum['counts'][1:2].values)
                       +int(euk_phylum['counts'][2:3].values)+int(euk_phylum['counts'][3:4].values)
                       +int(euk_phylum['counts'][4:5].values))

bac_diff = len(Bac) - (int(bac_order['counts'][:1].values)+int(bac_order['counts'][1:2].values)
                       +int(bac_order['counts'][2:3].values)+int(bac_order['counts'][3:4].values)
                       +int(bac_order['counts'][4:5].values))

vir_diff = len(Vir) - (int(vir_family['counts'][:1].values)+int(vir_family['counts'][1:2].values)
                       +int(vir_family['counts'][2:3].values)+int(vir_family['counts'][3:4].values)
                       +int(vir_family['counts'][4:5].values))

arc_diff = len(Arc) - (int(arc_phylum['counts'][:1].values)+int(arc_phylum['counts'][1:2].values))

euk_names = [str(euk_phylum['phylum'][:1].values),
...
vir_names = [str(vir_family['family'][:1].values),
             ...
             'Other']
arc_names = [str(arc_phylum['phylum'][:1].values),
...
euk_counts = [int(euk_phylum['counts'][:1]),
...
bac_counts = [int(bac_order['counts'][:1]),
...
vir_counts = [int(vir_family['counts'][:1]),
...
arc_counts = [int(arc_phylum['counts'][:1]),
...
tax_pie, text = ax.pie(group_size,
                      radius = r,
...
plt.setp(tax_pie, width = r-.5, edgecolor = 'white')
...
e, b, v, a = [plt.cm.Reds, plt.cm.Blues, plt.cm.Greens, plt.cm.Purples]
rank_pie, text = ax.pie(all_counts,
                     radius = r+.1,
                     labels = fix_names,
                     autopct = None,
                     labeldistance = 1.005,
                     colors = [e(0.6), e(0.5), e(0.4), e(0.3), e(0.2), e(0.1),
                              b(0.6), b(0.5), b(0.4), b(0.3), b(0.2), b(0.1),
                              v(0.6), v(0.5), v(0.4), v(0.3), v(0.2), v(0.1),
                              a(0.6), a(0.5)]#, a(0.4)]
                       )
...
plt.title('CACAO Taxon Contributions', 
          y = 1.25,fontsize = 32)
